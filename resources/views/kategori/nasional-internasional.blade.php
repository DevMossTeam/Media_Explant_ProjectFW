...
<main class="py-8">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Nasional dan Internasional</h1>

        @if ($news->count() > 0)
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 bg-white shadow rounded-lg overflow-hidden">
                    <a href="{{ route('nasional-internasional.detail', ['a' => $news->first()->id]) }}">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-gray-800 mt-2 text-justify">{{ Str::limit(strip_tags($news->first()->konten_berita), 150) }}</p>
                        <a href="{{ route('nasional-internasional.detail', ['a' => $news->first()->id]) }}" class="text-red-600 font-bold mt-4 inline-block">Baca Selengkapnya</a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach ($news->skip(1)->take(4) as $news)
                        <div class="bg-white shadow rounded-lg overflow-hidden">
                            <a href="{{ route('nasional-internasional.detail', ['a' => $news->id]) }}">
