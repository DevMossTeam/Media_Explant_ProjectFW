            ...
            @if ($buletins->isNotEmpty())
                <div class="grid grid-cols-1 gap-10 mt-6">
                    @foreach ($buletins as $buletin)
                        <a href="{{ route('buletin.show', $buletin->id) }}" class="relative block rounded-lg overflow-hidden shadow-md">
                    ...
                    <div>
                        <a href="{{ route('buletin.show', $buletin->id) }}" class="block">
                            <div class="relative rounded-lg overflow-hidden shadow-md mb-4">
                                <canvas id="pdf-viewer-terbaru-{{ $buletin->id }}" class="w-full h-80 object-cover rounded-lg"></canvas>
                            </div>
                        </a>
                        ...
                        <div class="flex items-center mt-1">
                            ...
                            <a href="{{ route('buletin.show', $buletin->id) }}" class="text-[#5773FF] text-lg font-medium">Lihat Buletin</a>
                        </div>
                    </div>
        ...
        @foreach ($buletinsRekomendasi as $buletin)
            <div>
                <a href="{{ route('buletin.show', $buletin->id) }}" class="block">
                    <div class="relative rounded-lg overflow-hidden shadow-md mb-2">
                        <canvas id="pdf-viewer-rekomendasi-{{ $buletin->id }}" class="w-full h-52 object-cover rounded-lg"></canvas>
                    </div>
                </a>
                ...
                <div class="flex items-center mt-1">
                    <i class="fa-solid fa-download text-[#5773FF] mr-2"></i>
                    <a href="{{ route('buletin.show', $buletin->id) }}" class="text-[#5773FF] text-lg font-medium">Unduh Sekarang</a>
                </div>
            </div>
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    loadingTask.promise.then(function(pdf) {
                        pdf.getPage(1).then(function(page) {
                            var canvas = document.getElementById('pdf-viewer-rekomendasi-{{ $buletin->id }}');
                            var context = canvas.getContext('2d');
