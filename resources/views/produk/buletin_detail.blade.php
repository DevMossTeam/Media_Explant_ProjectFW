...
@section('content')
    <div class="container mx-auto px-4 lg:px-16 xl:px-24 2xl:px-32 py-6 max-w-screen-2xl">
        ...
        <!-- Judul -->
        <h1 class="text-4xl font-bold italic">{{ $buletin->judul }}</h1>
        ...
        <div class="flex items-center text-sm text-gray-600 my-2">
            ...
            <span>{{ \Carbon\Carbon::parse($buletin->release_date)->translatedFormat('j F Y - H.i') }} WIB</span>
            ...
            <div class="ml-auto">
                <button id="bookmark-btn" class="flex items-center gap-2 text-gray-400 hover:text-gray-800"
                    data-item-id="{{ $buletin->id }}" data-bookmarked="{{ $isBookmarked ? 'true' : 'false' }}"
                    data-bookmark-type="Produk">
                    <span class="text-sm">
        ...
        <!-- Thumbnail PDF -->
        <div class="my-4">
            <img src="{{ $buletin->cover }}" alt="Thumbnail Buletin {{ $buletin->judul }}"
        ...
        <!-- Tanggapan -->
        <div class="mt-5" data-product-id="{{ $buletin->id }}">
        ...
        <div class="mt-10">
            ...
            <div id="rekomendasi">
                @include('produk.partials.BuletinRekomendasi', [
                    'rekomendasiBuletin' => $rekomendasiBuletin,
                ])
            </div>
        </div>
    </div>
        ...
        <div class="bg-[#212121] rounded-2xl p-6 w-full max-w-md relative">
            ...
            <div class="relative flex items-center">
                ...
                <div id="iconContainer"
                    ...
                    <!-- LinkedIn -->
                    <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(request()->fullUrl()) }}&title={{ urlencode($buletin->judul) }}"
                        target="_blank" class="flex flex-col items-center min-w-max">
                        <div class="bg-white p-3 rounded-full shadow-md hover:opacity-80 transition">
                    ...
                    <!-- Email -->
                    <a href="mailto:?subject={{ urlencode($buletin->judul) }}&body={{ urlencode(request()->fullUrl()) }}"
                        target="_blank" class="flex flex-col items-center min-w-max">
                        <div class="bg-white p-3 rounded-full shadow-md hover:opacity-80 transition">
    ...
    <div id="reportModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-20 hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96 relative">
            <h2 class="text-lg font-bold mb-4">Laporkan Buletin</h2>
            <form id="reportForm">
                <div class="mb-4">
    ...
    <div id="thankYouModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-30">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96 text-center">
            ...
            <p class="text-lg font-semibold mb-4">Terima kasih telah melaporkan buletin ini!</p>
            ...
            komentarForm.addEventListener('submit', async function(e) {
                ...
                try {
                    const response = await fetch("{{ route('komentar.kirim') }}", {
                        ...
                        body: JSON.stringify({
                            komentar: komentar,
                            item_id: "{{ $buletin->id }}", // Ubah sesuai dengan item_id buletin
                            komentar_type: "Produk", // Komentar untuk Buletin
                            parent_id: parentId
                        })
                    });
    ...
    `;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            bookmarkBtn.addEventListener('click', function(e) {
                ...
                const bookmarkType = this.dataset.bookmarkType; // Ambil jenis bookmark (Produk)
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('submitReportButton').addEventListener('click', function() {
                ...
                const additionalDetail = document.querySelector('textarea[name="detail_pesan"]').value
                    .trim();
                const itemId = new URLSearchParams(window.location.search).get('f');
                ...
                if (selectedReason && itemId) {
                    fetch('/report-news', {
                            ...
                            body: JSON.stringify({
                                ...
                                item_id: itemId,
                                pesan_type: 'Produk' // Change this based on the context
                            })
                        })
            ...
            function sendReaction(jenisReaksi) {
                fetch('/reaksi', {
                        ...
                        body: JSON.stringify({
                            ...
                            reaksi_type: 'Produk' // beda di sini!
                        })
                    })
                    .then(async response => {
