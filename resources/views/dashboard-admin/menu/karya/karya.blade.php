...
<div class="container mx-auto px-1 py-1">
    <div class="mb-6">
        ...
        <nav class="flex items-center text-sm text-gray-500 space-x-2" aria-label="Breadcrumb">
            ...
            <span class="text-gray-700 font-medium">Karya</span>
        </nav>
        <!-- Title -->
        <h1 class="mt-3 text-2xl font-bold text-gray-800">Daftar Karya</h1>
    </div>
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <form method="GET" id="perPageForm" class="flex items-center gap-2">
                ...
                <select name="perPage" id="perPage" onchange="this.form.submit()"
                    ...
                    <option value="1" {{ $perPage == 1 ? 'selected' : '' }}>5</option>
                ...
                <div class="relative group mr-2 inline-block">
                    <form method="GET" class="relative group mr-2 inline-block">
                        ...
                        <div id="filterDropdown"
                            ...
                            <!-- Status Karya -->
                            <div>
                                <p class="text-sm font-semibold mb-2">Status Karya</p>
                            ...
                            <div class="flex pt-2">
                                <div class="ml-auto">
                                    <button type="button"
                                        onclick="window.location.href='{{ route('admin.karya') }}?perPage={{ $perPage }}'"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-700 border border-gray-200 table-auto">
                <thead class="bg-gray-100 text-xs font-semibold text-gray-600">
                    <tr>
                        <th class="p-3 w-20">Media</th>
                        <th class="p-3">Judul Karya</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="p-3">
                            ...
                            <img src="{{ $karya->media_url ?? 'https://via.placeholder.com/48x48' }}" alt="Media"
                        ...
                        <td class="p-3">
                            <span
                                class="text-xs font-semibold px-2 py-1 rounded-full
                                    {{ $karya->visibilitas === 'public' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600' }}">
                                {{ ucfirst($karya->visibilitas) }}
                            </span>
                        </td>

                        <td class="p-3 text-sm text-gray-500">{{ $karya->release_date }}</td>
                        ...
                        <td class="p-3 flex justify-center space-x-2">
                            <div class="relative group">
                                <button
                                    ...
                                    onclick="window.location.href='{{ route('admin.karya.detail', $karya->id) }}'">
                                    <i class="fa fa-eye"></i>
                                </button>
                                <div
                            ...
                            <div class="relative group">
                                <button type="button"
                                    ...
                                    data-id="{{ $karya->id }}"
                                    data-url="{{ route('admin.karya.delete', ['id' => $karya->id]) }}">
                            ...
                            <!-- Hidden Form -->
                            <form id="delete-form-{{ $karya->id }}"
                                action="{{ route('admin.karya.delete', ['id' => $karya->id]) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
            ...
            <div class="flex items-center justify-between mt-4 text-sm text-gray-600">
                ...
                <div class="flex space-x-1">
                    ...
                    <button class="px-3 py-1 border rounded text-gray-600 hover:bg-gray-100"
                        onclick="window.location.href='{{ $karyas->previousPageUrl() }}'">
                        Previous
                    </button>
                    @endif

                    @for ($i = 1; $i <= $karyas->lastPage(); $i++)
                        @if ($i == $karyas->currentPage())
                        ...
                        <button class="px-3 py-1 border rounded text-gray-600 hover:bg-blue-100"
                            onclick="window.location.href='{{ $karyas->url($i) }}'">
                            {{ $i }}
                        </button>
                        @endif
                        @endfor
                        ...
                        <button class="px-3 py-1 border rounded text-gray-600 hover:bg-gray-100"
                            onclick="window.location.href='{{ $karyas->nextPageUrl() }}'">
                            Next
                        </button>
                        @else
