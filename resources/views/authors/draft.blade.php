...
<div class="container mx-auto mt-8">
    <div class="flex flex-col md:flex-row justify-between items-center mb-4">
        <div class="relative w-full md:w-2/3">
            <input type="text" id="searchInput" class="pl-10 pr-4 py-2 border rounded-lg w-full" placeholder="Cari berita...">
    ...
    <div id="publicationContainer">
        @if(isset($articles) && $articles->count() > 0)
            @foreach($articles as $article)
                <div class="flex items-start mb-4">
                    ...
                    <div class="w-40 h-20 mr-4">
                        @php
                            $firstImage = '';
                            if (preg_match('/<img.+src=[\'"](?P<src>.+?)[\'"].*>/i', $article->konten_berita, $image)) {
                                $firstImage = $image['src'];
                            }
                        @endphp
                        @if($firstImage)
                            <img src="{{ $firstImage }}" class="object-cover rounded-lg w-full h-full" alt="Gambar berita">
                        @else
            ...
            @endforeach
        @else
            <p class="text-gray-500 text-center">Tidak ada berita yang tersedia.</p>
        @endif
    </div>
